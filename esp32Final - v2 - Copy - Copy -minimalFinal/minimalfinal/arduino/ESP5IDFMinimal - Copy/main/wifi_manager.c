/**
 * @file wifi_manager.c
 * @brief WiFi connection and captive portal - FIXED portal startup
 * @version 1.2 - Fixed: Portal not starting when WiFi fails
 */
...
static bool netif_initialized = false;  // ✅ Track netif init
static bool event_loop_initialized = false;  // ✅ Track event loop init
...
void wifi_start_station(void)
{
    ...
    // ✅ Set DNS servers (Google DNS as backup)
    ...
    dns_info.ip.u_addr.ip4.addr = ESP_IP4TOADDR(8, 8, 4, 4);  // Google DNS backup
    esp_netif_set_dns_info(sta_netif, ESP_NETIF_DNS_BACKUP, &dns_info);
    
    ESP_LOGI(TAG, "✅ DNS servers configured (8.8.8.8, 8.8.4.4)");
    ...
    // ✅ Shorter timeout - fail faster if router is off
    ESP_LOGI(TAG, "⏳ Waiting for connection (timeout: %dms)...", WIFI_CONNECT_TIMEOUT_MS);
    ...
    if (!(bits & WIFI_CONNECTED_BIT)) {
        ESP_LOGE(TAG, "❌ WiFi Connection Failed! (Router offline or wrong credentials)");
        ESP_LOGW(TAG, "🔄 Stopping STA mode and starting portal...");
        
        // ✅ Clean up station mode
        esp_wifi_stop();
        esp_wifi_deinit();
...
{
    ESP_LOGI(TAG, "🔧 Starting WiFi Portal Mode...");
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        ESP_LOGI(TAG, "========================================");
        ESP_LOGI(TAG, "⚙️  WiFi Setup Portal Active");
        ESP_LOGI(TAG, "========================================");
...
void wifi_manager_init(void)
{
    ...
    if (wifi_configured && strlen(wifi_ssid) > 0) {
        ...
        ESP_LOGI(TAG, "⚙️  No WiFi credentials found - starting portal");
        wifi_start_portal();
    }
}
